...
[CustomEffect("A shader that inherits from PVShaderBase", "PVSB inheritors", "Ensoftener")]
public class TexturelessShader : CustomEffectBase, DrawTransform
{
    ...
    public bool isVertexShader, isPixelShader; public byte[] PixelShader, VertexShader; public EffectContext effectContext;
    ...
    public TexturelessShader(Guid? psGuid = null, Guid? vsGuid = null, Guid? vbGuid = null, bool usePS = true, bool useVS = false)
    ...
    /// <summary>A stream of structs assigned to semantics in vertex shaders. Streams can be created from arrays via
    /// <see cref="DataStream.Create"/> (set <b>canRead</b> and <b>canWrite</b> to true!).
    /// The extra int indicates the size of each struct in bytes.</summary>
    /// <remarks>A vertex buffer in vertex shaders works differently than a constant buffer in pixel shaders (in vertex HLSL, these are under <b>b1</b>).
    ...
    public virtual (DataStream stream, int bytesPerVertex) VsInBuffer => (null, 0);
    ...
    public new virtual void Dispose() { base.Dispose(); if (IsDisposed) return; dInfo?.Dispose(); transformGraph?.Dispose(); vertexBuffer?.Dispose(); }
    ...
    { return outputOpaqueSubRect = default; }
    ~TexturelessShader() => Dispose();
}
/// <inheritdoc/>
[CustomEffectInput("Source")] public class PVShaderBase : TexturelessShader
{
    public override RawRectangle MapInputRectanglesToOutputRectangle(RawRectangle[] inputRects, RawRectangle[] inputOpaqueSubRects, out RawRectangle outputOpaqueSubRect)
    { outputOpaqueSubRect = default; return inputRects[0]; }
    /// <inheritdoc/>
    public PVShaderBase(Guid? psGuid = null, Guid? vsGuid = null, Guid? vbGuid = null, bool usePS = true, bool useVS = false) : base(psGuid, vsGuid, vbGuid, usePS, useVS) { }
}
